        ...
        Pass
        {
            HLSLPROGRAM
                ...
                float4 FragEdge(Varyings i) : SV_Target
                {
                    ...
                    float IThis = GetLuma(_MainTex.Sample(sampler_LinearClamp, uv));
                    float IL    = abs(GetLuma(_MainTex.Sample(sampler_LinearClamp, uv + float2(-size.x, 0))) - IThis);
                    float IL2   = abs(GetLuma(_MainTex.Sample(sampler_LinearClamp, uv + float2(-size.x * 2, 0))) - IThis);
                    float IR    = abs(GetLuma(_MainTex.Sample(sampler_LinearClamp, uv + float2(size.x, 0))) - IThis);
                    float IT    = abs(GetLuma(_MainTex.Sample(sampler_LinearClamp, uv + float2(0, -size.y))) - IThis);
                    float IT2   = abs(GetLuma(_MainTex.Sample(sampler_LinearClamp, uv + float2(0, -size.y * 2))) - IThis);
                    float IB    = abs(GetLuma(_MainTex.Sample(sampler_LinearClamp, uv + float2(0, size.y))) - IThis);

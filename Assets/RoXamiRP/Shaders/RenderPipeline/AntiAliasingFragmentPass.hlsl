#ifndef ANTIALIASING_FRAGMENT_PASS_INCLUDE
#define ANTIALIASING_FRAGMENT_PASS_INCLUDE
...
float4 SampleCameraAttachment(float2 uv)
{
...
#define _relativeThreshold 0.125
	...
	#define _contrastThreshold 0.0625
	#define _relativeThreshold 0.166
#elif defined(_AA_LOW)
	#define _contrastThreshold 0.0833
	#define _relativeThreshold 0.250
#endif
...
float4 FXAA_Quality(Varyings IN) : SV_Target
{
    ...
    if (contrastMinMax < max(_contrastThreshold, maxLuma * _relativeThreshold))
    {
    ...
    float avrageLuma = 2 * (N + S + W + E) + NW + NE + SW + SE;
    avrageLuma /= 12;
    float contrastAvrage = abs(avrageLuma - M);
    float blendLuma = saturate(contrastAvrage / contrastMinMax);
    blendLuma = blendLuma * blendLuma;

    float vertical = abs(N + S - 2 * M) * 2 + abs(NW + SW - 2 * W) + abs(NE + SE - 2 * E);
    float horizantol = abs(W + E - 2 * M) * 2 + abs(SW + SE - 2 * S) + abs(NW + NE - 2 * N);
    bool isHorizantol = vertical > horizantol;

    float contrastPositive = abs((isHorizantol ? N : E) - M);
    float contrastNegative = abs((isHorizantol ? S : W) - M);

    float searchStart, searchEndThreshold;
    float2 blendDir = isHorizantol ?
        float2(0, texelSize.y) :
        float2(texelSize.x, 0);

    if (contrastPositive > contrastNegative)
    {
        searchStart = (M + (isHorizantol ? N : E)) * 0.5;
        searchEndThreshold = contrastPositive * 0.25;
    }
    else
    {
        searchStart = (M + (isHorizantol ? S : W)) * 0.5;
        searchEndThreshold = contrastNegative * 0.25;
        blendDir *= -1;
    }

    float2 searchStartUV = uv + blendDir * 0.5;
    float2 searchStepDir = isHorizantol ?
        float2(texelSize.x, 0) :
        float2(0, texelSize.y);
    ...
    for (i = 1; i <= maxStep; i++)
    {
        ...
        {
            searchStepPositive = isHorizantol ? step.x : step.y;
            break;
        }
    }
    if (i == maxStep + 1)
    {
        searchStepPositive = (isHorizantol ? searchStepDir.x : searchStepDir.y) * maxStep;
    }
    ...
    for (i = 1; i <= maxStep; i++)
    {
        ...
        {
            searchStepNegative = isHorizantol ? step.x : step.y;
            break;
        }
    }
    if (i == maxStep + 1)
    {
        searchStepNegative = (isHorizantol ? searchStepDir.x : searchStepDir.y) * maxStep;
    }
    ...
    float4 result = SampleCameraAttachment(uv + blendDir * blend);
...
#if defined(_AA_HIGH)
#define _contrastThreshold 0.04
#define _relativeThreshold 0.125
#elif defined(_AA_MIDDLE)
#define _contrastThreshold 0.05
#define _relativeThreshold 0.25
#elif defined(_AA_LOW)
#define _contrastThreshold 0.06
#define _relativeThreshold 0.25
#endif
...
float4 FXAA_Console(Varyings IN) : SV_Target
{
    ...
    float4 negative1 = SampleCameraAttachment(uv - dir1);
    float4 positive1 = SampleCameraAttachment(uv + dir1);
    ...
    float4 negative2 = SampleCameraAttachment(uv - dir2);
    float4 positive2 = SampleCameraAttachment(uv + dir2);
    float4 result2 = result1 * 0.5f + (negative2 + positive2) * 0.25f;

    float newLum = GetLuma(result2);
